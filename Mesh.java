	...
	private Vector3[] computeNormals () {
		...
		try {
			for (int i = 0; i < 3 * getNumFaces(); i += 3) {
				// Normale de la face courante
				// AB = B-A
				Vector3 AB = new Vector3(vertices[faces[i+1]]);
				AB.subtract(new Vector3(vertices[faces[i+0]]));
				// BC = C-B
				Vector3 BC = new Vector3(vertices[faces[i+2]]);
				BC.subtract(new Vector3(vertices[faces[i+1]]));
				// n = AB /\ BC
