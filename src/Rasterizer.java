    ...
    public void rasterizeFace (Fragment v1, Fragment v2, Fragment v3) {
        ...
        for (int i=xmin; i<=xmax; i++) {
        	for (int j=ymin; j<=ymax; j++) {
        		try {
					Vector coordBary = C.multiply(new Vector3(1,((double) i),((double) j)));
					if (coordBary.get(0) >= 0 && coordBary.get(1) >= 0 && coordBary.get(2) >= 0) {
						Fragment pixel = new Fragment(i, j);
						// Interpolation des attributs
						double alpha = coordBary.get(0);
						double beta = coordBary.get(1);
						double gamma = coordBary.get(2);
						for (int k=0; k<9; k++) {
							pixel.setAttribute(k, alpha*v1.getAttribute(k)+beta*v2.getAttribute(k)+gamma*v3.getAttribute(k));
						}
//						//DEPTH
//						pixel.setDepth(alpha*v1.getDepth()+beta*v2.getDepth()+gamma*v3.getDepth());
//						//COLOR
//						double r = alpha*v1.getColor().getRed() + beta*v2.getColor().getRed() + gamma*v3.getColor().getRed();
//						double g = alpha*v1.getColor().getGreen() + beta*v2.getColor().getGreen() + gamma*v3.getColor().getGreen();
//						double b = alpha*v1.getColor().getBlue() + beta*v2.getColor().getBlue() + gamma*v3.getColor().getBlue();
//						pixel.setColor(r/255,g/255,b/255);
//						//NORMAL
//						Vector3 nv1 = new Vector3(v1.getNormal());
//						nv1.scale(alpha);
//						Vector3 nv2 = new Vector3(v2.getNormal());
//						nv2.scale(beta);
//						Vector3 nv3 = new Vector3(v3.getNormal());
//						nv3.scale(gamma);
//						nv1.add(nv2);
//						nv1.add(nv3);
//						pixel.setNormal(new Vector3(nv1));;
//						// texture
//						pixel.setAttribute(7,alpha*v1.getAttribute(7)+beta*v2.getAttribute(7)+gamma*v3.getAttribute(7));
//						pixel.setAttribute(8,alpha*v1.getAttribute(8)+beta*v2.getAttribute(8)+gamma*v3.getAttribute(8));
						// Appel au shader
						shader.shade(pixel);
					}
